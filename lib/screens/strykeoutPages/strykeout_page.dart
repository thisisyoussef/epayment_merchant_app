...
import 'package:strykepay_merchant/screens/strykeoutPages/strykeout_error_page.dart';
import 'package:strykepay_merchant/screens/strykeoutPages/strykeout_success_page.dart';
...
class StrykeOutPage extends StatefulWidget {
  ...
  @override
  State<StrykeOutPage> createState() => _StrykeOutPageState();
}
...
class _StrykeOutPageState extends State<StrykeOutPage>
    with SingleTickerProviderStateMixin {
  void load() {
    setState(() {
      loading = true;
      StrykeOutPage.isTrying = true;
      controller.forward();
    });
  }
    ...
    _sub = linkStream.listen((String link) async {
      ...
      if (await transferConsent(
        ...
        setState(() {
          ...
          endLoad();
          StrykeOutPage.paymentSuccess = true;
        });
        return true;
      } else {
        StrykeOutPage.paymentFailure = true;
  ...
  void initState() {
    // _urlInit();
    StrykeOutPage.paymentFailure = false;
    StrykeOutPage.isTrying = false;
    StrykeOutPage.paymentSuccess = false;
    initUniLinks();
  ...
  Widget build(BuildContext context) {
    if (StrykeOutPage.paymentSuccess) {
/*      Navigator.push(
        context,
    ...
    return Scaffold(
      appBar: StrykeAppBar(
        popCallback: () {
          Provider.of<StrykeOutHandler>(context, listen: false)
              .setPaymentUrl("");
          Navigator.popAndPushNamed(context, HomePage.id);
        },
      ),
      body: LoadingOverlay(
        ...
        isLoading: Provider.of<StrykeOutHandler>(context, listen: true)
                    .getPaymentUrl() ==
                "" ||
            StrykeOutPage.isTrying,
        child: SafeArea(
          child: StrykeOutPage.paymentSuccess
              ? StrykeOutSuccessPage()
              : StrykeOutPage.paymentFailure && StrykeOutPage.isTrying == false
                  ? StrykeOutErrorPage()
                  : Column(
                       ...
                       */
                                        ...
                                        try {
                                          _launchURL(
                                              Provider.of<StrykeOutHandler>(
                                                      context,
                                                      listen: false)
                                                  .getPaymentUrl());
                                        } catch (e) {
                                          Navigator.pushNamed(
                                              context, StrykeOutErrorPage.id);
                                        }
                                        setState(() {
                                          controller.stop();
                                        });
